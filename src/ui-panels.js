...
function showSelectionPanel(unit) {
  ...
  const ut = UNIT_TYPES[unit.type] || { name: unit.type, cost: 0, combat: 0, rangedCombat: 0, range: 0, movePoints: 2, icon: '\u{2753}', class: 'civilian', desc: 'Unknown unit' };
  ...
  if (isPlayer) {
    ...
    const gpDef = GREAT_PEOPLE_TYPES.find(gp => gp.type === unit.type);
    if (gpDef) {
      const gpEntry = (game.greatPeopleEarned || []).find(g => g.type === unit.type && !g.used);
      if (gpEntry) {
        html += `<button class="sel-btn" style="border-color:#ffd700;background:rgba(255,215,0,0.1);font-weight:bold" onclick="activateGreatPersonFromUnit('${unit.type}')"><span>\u{2B50} Activate: ${gpDef.effect}</span></button>`;
      }
    }
    ...
    // Attack Barbarian Camp button — show if adjacent to or on a barbarian camp
    if (ut.combat > 0) {
      const adjNbs = getHexNeighbors(unit.col, unit.row);
      // Also check the unit's own tile
      adjNbs.push({ col: unit.col, row: unit.row });
      for (const nb of adjNbs) {
        const camp = (game.barbarianCamps || []).find(bc => !bc.destroyed && bc.col === nb.col && bc.row === nb.row);
        if (camp) {
          html += `<button class="sel-btn" style="border-color:#d9534f;background:rgba(217,83,79,0.1)" onclick="barbCampAction('${camp.id}','attack')"><span>\u{1F525} Attack Barbarian Camp</span></button>`;
          break;
        }
      }
    }
...
window.activateGreatPersonFromUnit = function(gpType) {
  const gpDef = GREAT_PEOPLE_TYPES.find(gp => gp.type === gpType);
  const gpEntry = (game.greatPeopleEarned || []).find(g => g.type === gpType && !g.used);
  if (gpDef && gpEntry) {
    useGreatPerson(gpEntry, gpDef);
    // Remove the unit from the map after activation
    game.units = game.units.filter(u => !(u.type === gpType && u.owner === 'player'));
    updateUI();
    render();
    deselectUnit();
  }
