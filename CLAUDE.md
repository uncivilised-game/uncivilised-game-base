...
- `.github/workflows/pr-merge.yml` — comment `/merge` on any PR targeting `devel` to add it to the auto-merge pipeline. Adds the `auto-merge` label and enables GitHub auto-merge (squash). Useful for including external contributor PRs that aren't on `fix/*` branches. Restricted to repo owners, members, and collaborators.
