...
| `scripts/feedback-digest.py` | Daily summary of player feedback, categorised and prioritised, posted as a GitHub issue |
...
- `.github/workflows/feedback-digest.yml` — daily (8am UTC) + manual. Fetches last 24h of player feedback from Supabase, generates an AI-categorised summary, and posts a `feedback-digest` GitHub issue. Supports `dry_run` and `lookback_hours` inputs. Runs `scripts/feedback-digest.py`.
