...
| `scripts/feedback-digest.py` | Daily morning email summarising player feedback, categorised and prioritised |
...
- `.github/workflows/feedback-digest.yml` — daily at 7am UTC + manual. Queries last 24h of player feedback from Supabase, categorises and prioritises it, generates a Claude summary, and emails the digest. Inputs: `lookback_hours`, `dry_run`, `digest_email`. Runs `scripts/feedback-digest.py`.
