...
| `scripts/feedback-digest.py` | Daily digest of new player feedback, posted as a GitHub issue |
...
- `.github/workflows/feedback-digest.yml` — daily (8am UTC) + manual. Queries new player feedback from Supabase, groups by category/priority, generates a Claude summary, and creates a GitHub issue. Marks processed feedback as "digested". Supports `dry_run` input. Runs `scripts/feedback-digest.py`.
