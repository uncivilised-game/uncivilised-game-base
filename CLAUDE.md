...
- `.github/workflows/conviction-close.yml` — daily (6am UTC) scan that auto-closes conviction issues resolved by merged PRs. Uses a two-phase approach: first marks issues as `pending-close` with a 24-hour grace period, then closes on the next run if no one objects. Uses direct PR cross-referencing + Claude semantic matching (with strict false-positive guards). Supports `dry_run` input via manual dispatch.
